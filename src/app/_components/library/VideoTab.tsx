  ...
  return (
    <>
      <PromptModal 
        ...
        message="Nhập tên cho mẫu tự động này (VD: Bản tin sáng):" 
              ...
              <div className="flex items-center gap-4 min-w-0">
                ...
                <div className="flex flex-col min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-2 text-xs font-semibold mt-1.5">
                    {(() => {
                      const estimatedSeconds = Math.max(15, (j.scriptData?.sections?.length || 3) * 5);
                      ...
                      const getRes = (r: string) => {
                        if (r === "9:16") return "1080x1920";
                        if (r === "16:9") return "1920x1080";
            ...
            {isExpanded && (
              <div className="flex flex-col xl:flex-row gap-6 p-6 border-t border-line/60 bg-black/20 animate-in slide-in-from-top-2 duration-300">
                <div className="flex-1 flex flex-col gap-4 max-w-full xl:max-w-[45%]">
                  <h3 className="text-base font-mono font-bold text-zinc-400 uppercase tracking-widest border-b border-line/50 pb-2">Thành phẩm Video</h3>
                  
                  {j.videoUrl && (
                    <div className="rounded-2xl overflow-hidden border border-line bg-black flex flex-col w-full">
                      <video src={j.videoUrl} controls className="w-full h-auto max-h-[500px]" />
                      <div className="p-3 bg-black flex justify-end">
                        <a href={j.videoUrl} download className="text-base font-mono text-hot bg-hot/10 px-3 py-1.5 rounded-lg border border-hot/20 hover:bg-hot/20">
                          ↓ Tải Video
                        </a>
                      </div>
                    </div>
