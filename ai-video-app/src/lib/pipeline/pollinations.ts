 ...
 */
export async function pollinationsGenerateImage(prompt: string, filename: string): Promise<string> {
  ...
  const url =
    ...
    `?width=768&height=1344&nologo=true&model=${encodeURIComponent(model)}&seed=${seed}`;
